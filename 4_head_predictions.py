...
def training_and_tuning_model_on_data(X_train, X_test, y_train, y_test, year, trails_num=1000):
    if not os.path.exists(f"results/model_params_head_prediction_for_{year}.json"):
        xgb = XGBClassifier()
        ...
        with open(f"results/model_params_head_prediction_for_{year}.json", "w") as file:
        ...
        with open(f"results/model_params_head_prediction_for_{year}.json", "r") as file:
...
def hits_calculations(model, test_content, directors_df, literals_df, year, candidates):
    ...
    hits = {"hits_1_head": xgb_hits_1_head,
...
def run_model(year: int, literals_df: pd.DataFrame, directors_df: pd.DataFrame, ma_df: pd.DataFrame, candidates: list):
    ...
    with open(f"results/hits_head_prediction_for_{year}.json", "w") as file:
        json.dump(hits, file)
