...
def process_df(df: pd.DataFrame, directors_df: pd.DataFrame, literals_df: pd.DataFrame, max_year: int):
    ...
    for head, tail in tqdm(zip(heads, tails)):
...
def dynamic_building_of_data(year, ma_df, directors_df, literals_df, candidates):
    ...
    X_train.to_csv("results/X_train.csv")
    X_test.to_csv("results/X_test.csv")
    pd.DataFrame(y_train).to_csv("results/y_train.csv")
    pd.DataFrame(y_test).to_csv("results/y_test.csv")
    pd.DataFrame(test_content).to_csv("results/test_content.csv")
...
def training_and_tuning_model_on_data(X_train, X_test, y_train, y_test, year, trails_num=1000):
    ...
    with open(f"results/model_params_for_{year}.json", "w") as file:
...
def run_model(year: int, literals_df: pd.DataFrame, directors_df: pd.DataFrame, ma_df: pd.DataFrame, candidates: list):
    ...
    #
    # with open(f"results/hits_for_{year}.json", "w") as file:
    #     json.dump(hits, file)
    #
...
def main():
    ...
    ma_df.drop("date", axis=1, inplace=True)
    ...
    year = 2012
    run_model(year, literals_df, directors_df, ma_df, CANDIDATES[str(year)])
