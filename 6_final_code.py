...
def process_df(df: pd.DataFrame, data_df: pd.DataFrame, mutual_directors: dict, weighted_mutual_directors: dict,
    ...
    if save:
        df.to_csv(join("example", "example_processed_df.csv"))
        pd.DataFrame({"heads": heads, "tails": tails}).to_csv(join("example", "example_heads_tails.csv"))
...
def get_data_prepared_ahead(candidates: list, directors_df: pd.DataFrame, literals_df: pd.DataFrame, max_year: int):
    ...
    """
    print(f"Year {max_year}\tStart Preparing the data")
    ...
    print(f"Year {max_year}\tEnded Preparing the data")
...
def dynamic_building_of_data(year, ma_df, candidates, data_df, mutual_directors,
    ...
    train_data = create_false_samples(train_data, candidates)
    test_data = create_false_samples(test_data, candidates)
...
def training_and_tuning_model_on_data(X_train, X_test, y_train, y_test, year, trails_num=1000):
    print(f"Year: {year}\tStart Training.")
    model_save_path = join("results", f"model_for_{year}.pkl")
...
def hits_calculations(model, test_content, data_df, mutual_directors, weighted_mutual_directors, candidates, year,
    ...
    for tup in test_content:
        ...
        labels += [1 if c == head else 0 for c in candidates]
        ...
        labels += [1 if c == tail else 0 for c in candidates]
...
def run_model(year: int, literals_df: pd.DataFrame, directors_df: pd.DataFrame, ma_df: pd.DataFrame, candidates: list):
    ...
    X_train, X_test, y_train, y_test, test_content = dynamic_building_of_data(year, ma_df, candidates, data_df,
                                                                              ...
                                                                              cik_to_idx,
                                                                              connection_comp_df)
    model = training_and_tuning_model_on_data(X_train, X_test, y_train, y_test, year, trails_num=TRIALS)
    hits, labels_and_scores = hits_calculations(model, test_content, data_df, mutual_directors,
                                                weighted_mutual_directors, candidates, year,
                                                cik_to_idx, connection_comp_df)

    with open(f"results/scores_{year}.json", "w") as file:
        json.dump(hits, file)

    with open(f"results/roc_details_{year}.pkl", "wb") as file:
        pickle.dump(labels_and_scores, file)
...
def main():
    ...
    for year in range_years:
        ...
        thread = threading.Thread(target=run_model,
                                  args=(year, literals_df, directors_df, ma_df, list(set(CANDIDATES[str(year)]))))
        thread.start()
