"""
Yuli Tshuva
Check that LGB doesn't yield better results than XGB
"""
...
def run_model(year: int, literals_df: pd.DataFrame, directors_df: pd.DataFrame, ma_df: pd.DataFrame, candidates: list,
    ...
    results_dir = f"LGB_results_{ratio}"
    ...
    model = training_and_tuning_model_on_data(X_train, X_test, y_train, y_test, year, results_dir, trails_num=TRIALS)
    ...
    with open(join(results_dir, f"scores_{year}.json"), "w") as file:
        json.dump(hits, file)

    with open(join(results_dir, f"roc_details_{year}.pkl"), "wb") as file:
...
def run_for_ratio(ratio):
    ...
    print(f"RATIO: {ratio}\tAll Done.")


def main():
    for ration in range(3, 16, 3):
        print("RATIO:", ration)
        run_for_ratio(ration)
        gc.collect()
